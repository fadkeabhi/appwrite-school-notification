    ...
    return(
        ...
        <div className='flex-1 flex flex-col bg-white m-8 rounded-lg'>
            <div className='flex p-5 w-full items-center justify-center text-lg font-medium gap-4'>
                <button onClick={()=>{navigate('/admin/student/add')}} className='flex px-3 py-2 rounded-lg bg-[#9BA4B5] gap-5'>
                ...
                <div className='grid text-white p-3 md:grid-cols-3 xl:grid-cols-4 gap-4 overflow-x-hidden overflow-y-auto'>
                {
                    studentsDetails.map((student)=>(
                        <div className='flex bg-[#394867] rounded-md'>
                            <div key={student._id} className='flex-1 text-white text-lg font-semibold flex flex-col p-4 rounded-md'>
                                <p>{student.email}</p>
                            ...
                            <button
                             onClick={()=>{
                                navigate(`/admin/student/edit?id=${student._id}&email=${student.email}`)
